...
public class ListLinkHandle implements LinkHandle {
	
	String url = null;
	HsqlDBUtil db = null; 
	
	public ListLinkHandle(HsqlDBUtil db){
		this.db = db;
	}
	...
	public int handle(String pid,String url,String siteid) throws SQLException{
		if(!isExit(url)){
			String sql = "INSERT INTO spider_resource (pid,url,type,httpstatus,ctime,save,crawl,parser,reqcount,siteid) VALUES (" + pid + ",'" + url + "',"+Filter.LIST+",200,'"+DatetimeUtil.getNow()+"',0,0,0,0,'"+siteid+"')";
	...
	private boolean isExit(String url) throws SQLException{
		String sql = "select id from spider_resource where url = '"+url+"'";
